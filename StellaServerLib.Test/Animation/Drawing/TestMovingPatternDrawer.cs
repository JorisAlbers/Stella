...
namespace StellaServerLib.Test.Animation.Drawing
{
    ...
    [TestFixture]
    public class TestMovingPatternDrawerWithoutDelta
    {
        [Test]
        public void Create_pattern_CorrectlyCreatesListOfFrames()
        {
            ...
            Color expectedColor2 = Color.FromArgb(4, 5, 6);
            Color expectedColor3 = Color.FromArgb(7, 8, 9);
            Color expectedEmptyColor = Color.Empty;
            ...
            int frameWaitMS = 100;
            int framesToTake = 6;
            MovingPatternDrawerWithoutDelta drawer = new MovingPatternDrawerWithoutDelta(lengthStrip, frameWaitMS, pattern);
            List<FrameWithoutDelta> frames = drawer.Take(framesToTake).ToList();

            //Assert
            //Frame 1, slide in
            FrameWithoutDelta frame = frames[0];
            Assert.AreEqual(0, frame.TimeStampRelative);
            Assert.AreEqual(expectedColor3, frame[0].Color);
            Assert.AreEqual(expectedEmptyColor, frame[1].Color);
            Assert.AreEqual(expectedEmptyColor, frame[2].Color);
            Assert.AreEqual(expectedEmptyColor, frame[3].Color);
            //Frame 2, slide in
            frame = frames[1];
            Assert.AreEqual(frameWaitMS, frame.TimeStampRelative);
            Assert.AreEqual(expectedColor2, frame[0].Color);
            ...
            Assert.AreEqual(frameWaitMS * 3, frame.TimeStampRelative);
            Assert.AreEqual(expectedColor1, frame[0].Color);
            Assert.AreEqual(expectedColor1, frame[1].Color);
            Assert.AreEqual(expectedColor2, frame[2].Color);
            Assert.AreEqual(expectedColor3, frame[3].Color);
            //Frame 5, slide out
            frame = frames[4];
            Assert.AreEqual(frameWaitMS * 4, frame.TimeStampRelative);
            Assert.AreEqual(expectedColor1, frame[0].Color);
            Assert.AreEqual(expectedColor1, frame[1].Color);
            Assert.AreEqual(expectedColor1, frame[2].Color);
            Assert.AreEqual(expectedColor2, frame[3].Color);
            //Frame 6, slide out
            frame = frames[5];
            Assert.AreEqual(frameWaitMS * 5, frame.TimeStampRelative);
            Assert.AreEqual(expectedColor1, frame[0].Color);
            Assert.AreEqual(expectedColor1, frame[1].Color);
            Assert.AreEqual(expectedColor1, frame[2].Color);
            Assert.AreEqual(expectedColor1, frame[3].Color);
        }
    }
}
