...
namespace rpi_ws281x
{
	...
	public class WS281x : IDisposable
	{
		...
		public WS281x(Settings settings)
		{
			...
			if(settings.Channels[0] != null)
			{
				InitChannel(0, settings.Channels[0]);
			}
		...
		public void Render()
		{
			for(int i=0; i<= Settings.Channels.Length -1; i++)
			{
				if (Settings.Channels[i] != null)
				{
					...
					Marshal.Copy(ledColor, 0, _ws2811.channel_0.leds, ledColor.Count());
				}
			}
		...
		{
			_ws2811.channel_0.count			= channelSettings.LEDs.Count;
			_ws2811.channel_0.gpionum		= channelSettings.GPIOPin;
			_ws2811.channel_0.brightness	= channelSettings.Brightness;
			_ws2811.channel_0.invert		= Convert.ToInt32(channelSettings.Invert);
			...
			if(channelSettings.StripType != StripType.Unknown)
			{
				...
				_ws2811.channel_0.strip_type = (int)channelSettings.StripType;
			}
		}
