...
namespace StellaServerLib.Animation.Drawing.Fade
{
    ...
    public class RandomFadeDrawerWithoutDelta : IDrawerWithoutDelta
    {
        private readonly Color[] _pattern;
        private readonly Color[][] _fadePatterns;
        private readonly int _stripLength;
        ...
        public RandomFadeDrawerWithoutDelta(int stripLength, int frameWaitMS, Color[] pattern, int fadeSteps)
        {
            _stripLength = stripLength;
            _frameWaitMS = frameWaitMS;
            _pattern = pattern;
        ...
        public IEnumerator<FrameWithoutDelta> GetEnumerator()
        {
            ...
            while (true)
            {
                ...
                // draw existing FadePoints
                FrameWithoutDelta frame = new FrameWithoutDelta(frameIndex++, timestampRelative += _frameWaitMS, _stripLength);
                DrawFadePoints(fadePointsPerFadeStep, frame);
        ...
        private void DrawFadePoints(LinkedList<List<FadePoint>> fadePointsPerFadeStep, FrameWithoutDelta frame)
        {
            foreach (List<FadePoint> fadePoints in fadePointsPerFadeStep)
            {
                // All fade points at this index have the same fade step count.
                ...
                foreach (FadePoint fadePoint in fadePoints)
                {
                    for (int i = 0; i < pattern.Length; i++)
                    {
                        ...
                        frame[pixelIndex] = new PixelInstructionWithoutDelta(pattern[i]);
                    }
                    fadePoint.Step++;
                }
            }
